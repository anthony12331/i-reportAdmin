    ...
    <section style={styles.reviewPanel}>
      ...
      {src ? (
        ...
        <div style={styles.emptyMedia}>
          ...
          <span style={styles.emptyMediaText}>No {label.toLowerCase()} available</span>
        </div>
      )}
    </section>
  );
...
export const UserImagePreviewModal = memo(function UserImagePreviewModal({ src, onClose }) {
  if (!src) return null;

  return (
    <div style={styles.darkOverlay} onClick={onClose}>
      <div style={styles.previewShell} onClick={(event) => event.stopPropagation()}>
        <img src={src} style={styles.previewImage} alt="Preview" />
        <button type="button" onClick={onClose} style={styles.previewCloseButton}>
      ...
      <div style={styles.reviewModal} onClick={(event) => event.stopPropagation()}>
        <div style={styles.reviewHeader}>
          <div>
            ...
            <p style={styles.reviewSubtitle}>Inspect the selfie, ID photo, and profile details in one place.</p>
          </div>
    ...
    <div style={styles.staticOverlay}>
      ...
      <div style={styles.detailsModal}>
        <div style={styles.detailsHeader}>
          <div>
            <h2 style={styles.detailsTitle}>{user.first_name} {user.last_name}</h2>
            <p style={styles.detailsSubtitle}>Verified citizen profile and management</p>
          </div>
            ...
            <aside style={styles.profileCard}>
              <div style={styles.profileCardHeader}>
                <div>
                  ...
                  <p style={styles.profileSubtitle}>{user.position || 'Verified Citizen'}</p>
                </div>
              ...
              <button
                type="button"
                style={{ ...styles.profilePhotoBtn, cursor: profileImageUrl ? 'zoom-in' : 'default' }}
                onClick={() => profileImageUrl && onOpenPreview(profileImageUrl)}
                aria-label="View large selfie"
              >
                {profileImageUrl ? (
                  <img src={profileImageUrl} alt="Selfie" style={styles.profilePhoto} />
                ) : (
              ...
              {idPhotoUrl && (
                ...
                >
                  ...
                  <img src={idPhotoUrl} alt="ID Photo" style={styles.profilePhoto} />
                </button>
              )}
            </aside>
      ...
      <div style={styles.suspendedPopupBox}>
        <div style={styles.popupHeader}>
          <div>
            ...
            <p style={styles.popupSubtitle}>Manage users who have lost verification access.</p>
          </div>
          <button type="button" onClick={onClose} style={styles.closeButton}>
            <X size={18} />
          </button>
              ...
              {users.map((user) => (
                <div key={user.id} style={styles.suspendedItem}>
                  <div>
                    ...
                    <p style={styles.suspendedMeta}>ID #{user.user_id || 'N/A'} · {user.email || 'No email'}</p>
                  </div>
                  <button type="button" style={styles.viewBtn} onClick={() => onViewUser(user)}>
                    View
                  </button>
                </div>
              ))}
    ...
    <div style={styles.promptOverlay}>
      <div style={styles.promptBox}>
        <div style={styles.promptHeader}>
          ...
          <p>Enter a reason to log and confirm the suspension for {user.first_name} {user.last_name}.</p>
        </div>
        <textarea
          value={message}
        ...
        <p style={styles.dangerText}>Suspended users cannot access the system until their verification is restored.</p>
      </div>
    </div>
  );
});
