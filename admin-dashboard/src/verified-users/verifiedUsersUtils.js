...
export function getVerifiedUserDetails(user) {
  ...
  const detailRows = [
    { label: 'Full Name', value: user.extension ? `${fullName}, ${user.extension}` : fullName },
  ...
  const extraFields = Object.entries(user)
    ...
    .filter(([key, value]) => !DETAIL_IGNORED_KEYS.has(key) && value != null && value !== '' && typeof value !== 'object')
    .map(([key, value]) => ({
      label: formatFieldLabel(key),
      value: formatBooleanValue(value),
    }));

  const combined = [...detailRows, ...extraFields]
