...
const getDialogTone = (message, variant) => {
  ...
  if (text.includes('success') || text.includes('successful') || text.includes('complete') || text.includes('verified')) {
    return {
      icon: CheckCircle2,
      accent: '#10b981',
      title: 'System Notice',
  ...
  if (text.includes('error') || text.includes('failed') || text.includes('denied') || text.includes('security')) {
    return {
      icon: AlertTriangle,
      accent: '#ef4444',
      title: 'System Alert',
...
function MessageDialog({ dialog, onClose }) {
  ...
  return (
    <div style={styles.overlay} role="presentation" onMouseDown={() => isConfirm && onClose(false)}>
      <div
      ...
      >
        <div style={styles.header}>
          <div style={{ ...styles.iconWrap, backgroundColor: `${tone.accent}18`, color: tone.accent }}>
            <Icon size={24} />
          </div>
          <button type="button" style={styles.closeBtn} onClick={() => onClose(isConfirm ? false : true)} aria-label="Close message">
            <X size={18} />
          </button>
        </div>

        <h2 id="message-box-title" style={styles.title}>{dialog.title || tone.title}</h2>
        <p style={styles.message}>{String(dialog.message || '')}</p>

        <div style={styles.actions}>
          {isConfirm && (
            <button type="button" style={styles.secondaryBtn} onClick={() => onClose(false)}>
